...
Есть избыточность колонок в обеих таблицах, хотя на практике достаточно хранить "tg_id" и через него доставать всё остальное. Но мне так удобно потом анализировать через pandas всякое.
### Таблица `users`
